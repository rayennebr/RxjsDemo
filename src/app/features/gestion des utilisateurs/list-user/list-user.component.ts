...
export class ListUserComponent implements OnInit {
  ...
  getAllUsers(): void {
    this.userService$
      ...
      .pipe(
        ...
        retry(2),
        finalize(() =>
          console.log('finalize: Observable fermé (par completion ou erreur)')
        )
      )
      .subscribe({
        next: (res) => {
  ...
  loadUsersWithPost() {
    forkJoin({
      users: this.httpClient.get<any[]>(
        'https://jsonplaceholder.typicode.com/users'
      ),
      posts: this.httpClient.get<any[]>(
  ...
  loadData() {
    forkJoin({
      ...
      comments: this.httpClient.get<any[]>(
      ...
      .subscribe({
        next: (result) => {
          console.log('Users with posts and comments:', result);
        },
        error: (err) => {
